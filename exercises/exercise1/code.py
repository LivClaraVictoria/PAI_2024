...
#remember: test data has no noise! during training you can 
...
class Model(object):
    ...
    def __init__(self):
        ...
        #1.0 acts as an instance of constantkernel(), so it creates an extra tunable hyperparameter
        self.kernel = 1.0*WhiteKernel() + 1.0*Matern()
    ...
    def generate_predictions(self, test_coordinates: np.ndarray, test_area_flags: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        predictions = gp_mean
        predictions[test_area_flags == 1] += gp_std[test_area_flags==1]*norm.ppf(c1/(c1+c2))
    ...
    def train_model(self, train_targets: np.ndarray, train_coordinates: np.ndarray, train_area_flags: np.ndarray):
        ...
        # randomly sample 70% of indeces, using given rng
        n = train_targets.shape[0]
        sample_rate = 0.5
        idx_array = np.arange(n)
        num_samples = int(sample_rate*n)
        print(f"array shape: {n}, num samples: {num_samples}. ")

        sampled_idx = self.rng.choice(idx_array, size=num_samples ,replace=False) #used rng to pick idx to make results consistent

        print("\nBalancedness of data pre sampling:")       
        print(f" > Percentage of 1s: {(len([i for i in train_area_flags if i == 1]) / n) * 100:.2f}%")
        print(f" > Percentage of 0s: {(len([i for i in train_area_flags if i == 0]) / n) * 100:.2f}%")
        print("-------------------------------------")
        print("\nBalancedness of data post sampling:")
        ones = len([train_targets[i] for i in sampled_idx if train_area_flags[i] == 1])
        zeros = len([train_targets[i] for i in sampled_idx if train_area_flags[i] == 0])

        
        print(f" > Percentage of 1s: {(ones / num_samples) * 100:.2f}%")
        print(f" > Percentage of 0s: {(zeros / num_samples) * 100:.2f}%")

        train_sampled = train_coordinates[sampled_idx]
        train_targets_samples = train_targets[sampled_idx]

        print("now training........ fingers crossed")
        self.model.fit(train_sampled, train_targets_samples)
        print("DONE")
...
# train_x: array of [coo1, coo2, area flag]
...
def main():
    ...
    train_x = np.loadtxt(data_location / 'train_x.csv', delimiter=',', skiprows=1)
    train_y = np.loadtxt(data_location / 'train_y.csv', delimiter=',', skiprows=1)
    test_x = np.loadtxt(data_location / 'test_x.csv', delimiter=',', skiprows=1)
    ...
    if EXTENDED_EVALUATION:
        execute_extended_evaluation(model, output_dir=data_location)
