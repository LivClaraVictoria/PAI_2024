...
def generate_sample(n_samples, seed=None):
    ...
    return dist.rvs(n_samples, random_state=random_state) # returns samples. rvs is a scipy method
...
def log_posterior_probs(x):
    ...
    for i, dist in enumerate(HYPOTHESIS_SPACE):
        log_p[i] = sum(dist.logpdf(x))

    log_p = log_p + np.log(PRIOR_PROBS)

    # calculate log_p(X) using logSumExp trick
    normConst = logsumexp(log_p)

    # subtract from each term in array
    log_p = log_p-normConst
...
def main():
    ...
    dist = HYPOTHESIS_SPACE[1] #laplace
